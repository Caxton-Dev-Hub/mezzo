...
import type { DisputeReasonCode, EscrowState } from '@mezzo/shared-types';
...
export function RaiseDisputeModal({
  ...
  const evidenceRequired = escrowState === 'DELIVERED';
  ...
  const canSubmit =
    ...
    (!evidenceRequired || confirmedEvidenceCount > 0);

  return (
    <Modal open={open} onClose={onClose} title="Raise a dispute" className="max-w-xl">
      <p className="text-sm text-fog">
        {evidenceRequired
          ? "Raising a dispute freezes this escrow until an arbiter reviews it. You'll need at least one photo or video showing the problem."
          : "Raising a dispute freezes this escrow until an arbiter reviews it. Since delivery hasn't been confirmed, describe what happened below — photo evidence is optional at this stage."}
      </p>
      ...
      <div className="mt-4">
        <Label>Evidence{evidenceRequired ? '' : ' (optional)'}</Label>
        <EvidenceCapture
          escrowId={escrowId}
          phase="AT_DELIVERY"
