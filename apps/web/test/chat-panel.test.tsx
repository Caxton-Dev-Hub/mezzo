...
vi.mock('../lib/chat-client', () => ({
  ...
  markChatRead: vi.fn(async () => ({ userId: BUYER_ID, lastReadAt: new Date() })),
}));
...
function makeEscrow(state: EscrowState = 'FUNDED'): EscrowDetailResponse {
  return {
    ...
    parties: [
      { userId: BUYER_ID, role: 'BUYER' as EscrowRole, termsAcceptedAt: new Date() },
      { userId: SELLER_ID, role: 'SELLER' as EscrowRole, termsAcceptedAt: new Date() },
    ],
...
describe('ChatPanel', () => {
  ...
  it('denies a non-party access to the chat', async () => {
    useAuthStore.setState({
      ...
      accessToken: 'token',
      user: { id: STRANGER_ID, email: 'stranger@example.com', role: 'USER', createdAt: new Date() },
    });

    renderWithProviders(<ChatPanel escrow={makeEscrow()} />);

    expect(await screen.findByText('You do not have access to this conversation.')).toBeInTheDocument();
  });
  ...
  it('shows an optimistic message immediately and reconciles it with the server copy', async () => {
    ...
    renderWithProviders(<ChatPanel escrow={makeEscrow()} />);
    await waitFor(() => expect(listeners?.connect?.length ?? 0).toBeGreaterThan(0));
    await waitFor(() => expect(emitted.some((e) => e.event === 'message:read')).toBe(true));
  ...
  it('marks a send as failed and retryable when the socket is not connected', async () => {
    ...
    expect(await screen.findByText('Failed to send — retry')).toBeInTheDocument();
  });
});
